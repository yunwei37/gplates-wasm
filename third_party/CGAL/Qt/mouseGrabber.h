...
#ifndef QOpenGLVIEWER_MOUSE_GRABBER_H
#define QOpenGLVIEWER_MOUSE_GRABBER_H
...
/*! \brief Abstract class for objects that grab mouse focus in a CGAL::QOpenGLViewer.
  \class MouseGrabber mouseGrabber.h CGAL::QOpenGLViewer/mouseGrabber.h
  ...
  CGAL::QOpenGLViewers parse this pool, calling all the MouseGrabbers' checkIfGrabsMouse()
  methods that setGrabsMouse() if desired.

  When a MouseGrabber grabsMouse(), it becomes the CGAL::QOpenGLViewer::mouseGrabber().
  All the mouse events (mousePressEvent(), mouseReleaseEvent(),
  mouseMoveEvent(), mouseDoubleClickEvent() and wheelEvent()) are then
  transmitted to the CGAL::QOpenGLViewer::mouseGrabber() instead of being normally
  ...
  MouseGrabber in a specific CGAL::QOpenGLViewer using
  CGAL::QOpenGLViewer::setMouseGrabberIsEnabled().

  <h3>Implementation details</h3>

  In order to make MouseGrabber react to mouse events, mouse tracking has to be
  activated in the CGAL::QOpenGLViewer which wants to use MouseGrabbers: \code init() {
  ...
  the CGAL::QOpenGLViewer::camera() of the CGAL::QOpenGLViewer that uses the MouseGrabber. It can be
...
public:
  ...
  //@{
public:
  /*! Pure virtual method, called by the CGAL::QOpenGLViewers before they test if the
  ...
  corresponding to the calling CGAL::QOpenGLViewer Camera. Project on screen and then
  ...
  /*! Returns \c true when the MouseGrabber grabs the CGAL::QOpenGLViewer's mouse events.
...
public:
  ...
  Used by the CGAL::QOpenGLViewer to parse all the MouseGrabbers and to check if any of
...
*/
  ...
  removeFromMouseGrabberPool(), the CGAL::QOpenGLViewers no longer checkIfGrabsMouse() on
...
#endif // QOpenGLVIEWER_MOUSE_GRABBER_H
