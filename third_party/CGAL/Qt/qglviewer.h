...
*****************************************************************************/
// $URL: https://github.com/CGAL/cgal/blob/releases/CGAL-4.14/GraphicsView/include/CGAL/Qt/qglviewer.h $
// $Id: qglviewer.h bace158 %aI Jane
// SPDX-License-Identifier: GPL-3.0

#ifndef QOpenGLVIEWER_QOpenGLVIEWER_H
#define QOpenGLVIEWER_QOpenGLVIEWER_H
...
\class QOpenGLViewer qglviewer.h QOpenGLViewer/qglviewer.h
...
To use a QOpenGLViewer, derive you viewer class from the QOpenGLViewer and overload its
...
QOpenGLViewer (Qt's callback mechanism). See the <a
href="../examples/callback.html">callback example</a> for a complete
implementation.

\nosubgrouping */
class CGAL_QT_EXPORT QOpenGLViewer : public QOpenGLWidget, public QOpenGLFunctions {
  Q_OBJECT

public:  
  explicit QOpenGLViewer(QOpenGLContext* context, QWidget *parent = 0,
                     ::Qt::WindowFlags flags = 0);
  explicit QOpenGLViewer(QWidget *parent = 0,
                     ::Qt::WindowFlags flags = 0);

  virtual ~QOpenGLViewer();
...
public:
  ...
  glGet() does. Instead, it returns the QOpenGLViewer internal variable. If you
...
public:
  ...
  the mouseGrabber() of this QOpenGLViewer. This is useful when you use several
  ...
  completely disable a MouseGrabber in all the QOpenGLViewers. */
  bool
...
public:
  ...
  Note that if the QOpenGLViewer is embedded in an other QWidget, it returns \c true
  when the top level widget is in full screen mode. */
  bool isFullScreen() const { return fullScreen_; }
  
  /*! Returns the recommended size for the QOpenGLViewer. Default value is 600x400
   * pixels. */
...
public:
  ...
  // These methods are part of the QOpenGLWidget public API.
...
Q_SIGNALS:
  ...
  /*! Signal emitted at the end of the QOpenGLViewer::paintGL() method, when frame
  is drawn.
  ...
  /*! Signal emitted by the default QOpenGLViewer::help() method.
...
public Q_SLOTS:
  virtual void help();
  virtual void aboutQOpenGLViewer();
...
protected:
  ...
  this method, as it will result in an infinite loop. The different QOpenGLViewer
  ...
  If you build a class that inherits from QOpenGLViewer, this is the method you want
...
protected:
  ...
  /*! @name QOpenGLViewer pool */
  //@{
public:
  /*! Returns a \c QList that contains pointers to all the created QOpenGLViewers.
  ...
  Can be useful to apply a method or to connect a signal to all the viewers:
    \code
  foreach (QOpenGLViewer* viewer, QOpenGLViewer::QOpenGLViewerPool())
    connect(myObject, SIGNAL(IHaveChangedSignal()), viewer, SLOT(update()));
  \endcode
*/
  static QList<QOpenGLViewer *> &QOpenGLViewerPool();

  /*! Returns the index of the QOpenGLViewer \p viewer in the QOpenGLViewerPool(). This
  index in unique and can be used to identify the different created QOpenGLViewers
  (see stateFileName() for an application example).

  When a QOpenGLViewer is deleted, the QOpenGLViewers' indexes are preserved and NULL is
  set for that index. When a QOpenGLViewer is created, it is placed in the first
  available position in that list. Returns -1 if the QOpenGLViewer could not be
  found (which should not be possible). */
  static int QOpenGLViewerIndex(const QOpenGLViewer *const viewer) {
    return QOpenGLViewer::QOpenGLViewerPool().indexOf(const_cast<QOpenGLViewer *>(viewer));
  }
//@}
...
private:
  ...
  QOpenGLViewer(const QOpenGLViewer &v);
  QOpenGLViewer &operator=(const QOpenGLViewer &v);
...
#endif // QOpenGLVIEWER_QOpenGLVIEWER_H
