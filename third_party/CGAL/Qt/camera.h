...
*****************************************************************************/
// $URL: https://github.com/CGAL/cgal/blob/releases/CGAL-4.14/GraphicsView/include/CGAL/Qt/camera.h $
// $Id: camera.h 93d9e8e %aI Jane
// SPDX-License-Identifier: GPL-3.0


#ifndef QOpenGLVIEWER_CAMERA_H
#define QOpenGLVIEWER_CAMERA_H
#include <QMap>
#include <QDomElement>
...
/*! \brief A perspective or orthographic camera.
  \class Camera camera.h CGAL::QOpenGLViewer/camera.h
  ...
  actions is defined by the CGAL::QOpenGLViewer::MouseAction enum. Use
  CGAL::QOpenGLViewer::setMouseBinding() to attach a specific action to an arbitrary mouse
  button-state key binding. These actions are detailed in the <a
  href="../mouse.html">mouse page</a>.

  The default button binding are: CGAL::QOpenGLViewer::ROTATE (left), CGAL::QOpenGLViewer::ZOOM
  (middle) and CGAL::QOpenGLViewer::TRANSLATE (right). With this configuration, the Camera
  \e observes a scene and rotates around its pivotPoint(). You can switch
  between this mode and a fly mode using the CGAL::QOpenGLViewer::CAMERA_MODE (see
  CGAL::QOpenGLViewer::toggleCameraMode()) keyboard shortcut (default is 'Space').
  ...
  from CGAL::QOpenGLViewer::sceneRadius(), CGAL::QOpenGLViewer::sceneCenter() and
  zClippingCoefficient() by the zNear() and zFar() methods. Reasonable values on
  the scene extends hence have to be provided to the CGAL::QOpenGLViewer in order for the
  ...
  A Camera can also be used outside of a CGAL::QOpenGLViewer or even without OpenGL for
...
public:
  ...
  When the Camera is attached to a CGAL::QOpenGLViewer, these values and hence the
  ...
  CGAL::QOpenGLViewer's window dimensions when the Camera is attached to a CGAL::QOpenGLViewer. See
  ...
  \attention When CGAL::QOpenGLViewer::cameraPathAreEdited(), this value is set to 5.0 so
...
public Q_SLOTS:
  ...
  a CGAL::QOpenGLViewer. It actually sets the screenHeight() to 100 and the screenWidth()
  ...
  CGAL::QOpenGLViewer::draw() method. */
...
public:
  ...
  Note that CGAL::QOpenGLViewer::sceneRadius() (resp. CGAL::QOpenGLViewer::setSceneRadius()) simply
  call this method (resp. setSceneRadius()) on its associated
  CGAL::QOpenGLViewer::camera(). */
  ...
  Note that CGAL::QOpenGLViewer::sceneCenter() (resp. CGAL::QOpenGLViewer::setSceneCenter()) simply
