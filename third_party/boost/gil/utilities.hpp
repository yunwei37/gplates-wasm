...
struct deref_base : public std::__unary_function<ArgType, ResultType> {
...
template <typename T> 
struct identity : public std::__unary_function<T,T> {
...
template <typename T>
struct inc : public std::__unary_function<T,T> {
...
/// \brief operator-- wrapped in a function object
template <typename T>
struct dec : public std::__unary_function<T,T> {
    T operator()(T x) const { return --x; }
};
