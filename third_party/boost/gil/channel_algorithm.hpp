...
struct channel_converter_unsigned_impl : public std::__unary_function<DstChannelV,SrcChannelV> {
...
template <typename DstChannelV> struct channel_converter_unsigned<bits32f,DstChannelV> : public std::__unary_function<bits32f,DstChannelV> {
...
template <typename SrcChannelV> struct channel_converter_unsigned<SrcChannelV,bits32f> : public std::__unary_function<SrcChannelV,bits32f> {
    bits32f operator()(SrcChannelV   x) const { return bits32f(x/float(channel_traits<SrcChannelV>::max_value())); }
};

template <> struct channel_converter_unsigned<bits32f,bits32f> : public std::__unary_function<bits32f,bits32f> {
    bits32f operator()(bits32f   x) const { return x; }
};


/// \brief 32 bit <-> float channel conversion
template <> struct channel_converter_unsigned<bits32,bits32f> : public std::__unary_function<bits32,bits32f> {
...
/// \brief 32 bit <-> float channel conversion
template <> struct channel_converter_unsigned<bits32f,bits32> : public std::__unary_function<bits32f,bits32> {
...
template <> struct channel_convert_to_unsigned<bits8s> : public std::__unary_function<bits8s,bits8> { 
    typedef bits8 type;
    type operator()(bits8s  val) const { return val+128; } 
};

template <> struct channel_convert_to_unsigned<bits16s> : public std::__unary_function<bits16s,bits16> { 
    typedef bits16 type;
    type operator()(bits16s  val) const { return val+32768; } 
};

template <> struct channel_convert_to_unsigned<bits32s> : public std::__unary_function<bits32s,bits32> {
    typedef bits32 type;
...
template <> struct channel_convert_from_unsigned<bits8s> : public std::__unary_function<bits8,bits8s> { 
    typedef bits8s type;
    type  operator()(bits8  val) const { return val-128; } 
};

template <> struct channel_convert_from_unsigned<bits16s> : public std::__unary_function<bits16,bits16s> { 
    typedef bits16s type;
    type operator()(bits16 val) const { return val-32768; } 
};

template <> struct channel_convert_from_unsigned<bits32s> : public std::__unary_function<bits32,bits32s> {
...
template <typename SrcChannelV, typename DstChannelV> // Model ChannelValueConcept
struct channel_converter : public std::__unary_function<SrcChannelV,DstChannelV> {
