/* $Id: PlatesFormatUtils.cc 16827 2015-11-19 07:02:48Z jcannon $ */
...
namespace
{
			...
			const GPlatesUtils::UnicodeString &inactive_data_type_code)
	{
		...
		{
			return is_active_property_value.get()->value() ? active_data_type_code : inactive_data_type_code;
		}
			...
			const GPlatesModel::FeatureHandle::const_weak_ref &feature)
	{
		...
		if (dipslip_property_value)
		{
			...
			if (dipslip_enumeration_type.is_equal_to(dipslip_property_value.get()->type()))
			{
				if (dipslip_enumeration_value_compression.is_equal_to(dipslip_property_value.get()->value()))
				{
					return "NF";
				}
				if (dipslip_enumeration_value_extension.is_equal_to(dipslip_property_value.get()->value()))
				{
					...
					if (subcategory_property_value)
					{
						...
						if (subcategory_property_value.get()->value().is_equal_to(thrust_string))
						{
							return "TH";
						}
					}
		...
		if (strike_slip_property_value)
		{
			...
			if (strike_slip_enumeration_type.is_equal_to(strike_slip_property_value.get()->type()))
			{
				if (strike_slip_enumeration_value_unknown.is_equal_to(strike_slip_property_value.get()->value()))
				{
					return "SS";
				}
			}
			...
			const GPlatesModel::FeatureHandle::const_weak_ref &feature)
	{
		...
		if (subduction_polarity_property_value)
		{
			...
			if (subduction_polarity_enumeration_type.is_equal_to(subduction_polarity_property_value.get()->type()))
			{
				if (subduction_polarity_enumeration_value_left.is_equal_to(
						subduction_polarity_property_value.get()->value()))
				{
					return "sL";
				}
		...
		if (is_active_property_value)
		{
			return is_active_property_value.get()->value() ? "TR" : "XT";
		}
